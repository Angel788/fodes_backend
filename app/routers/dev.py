...
SEED_USERS = [
    {"nombre": "Seed Objetivo",  "correo": f"objetivo{_SEED_MARKER}"},
    {"nombre": "Seed Reporter1", "correo": f"reporter1{_SEED_MARKER}"},
    {"nombre": "Seed Reporter2", "correo": f"reporter2{_SEED_MARKER}"},
    {"nombre": "Seed Votante1",  "correo": f"votante1{_SEED_MARKER}"},
    {"nombre": "Seed Votante2",  "correo": f"votante2{_SEED_MARKER}"},
]
...
def seed_moderation(db: Session = Depends(get_db), _=Depends(_check_secret)):
    ...
    for u in SEED_USERS:
        db.execute(text("""
            INSERT IGNORE INTO usuarios (nombre, correo, password, status, strikes_count)
            VALUES (:nombre, :correo, :pw, 'NORMAL', 0)
        """), {"nombre": u["nombre"], "correo": u["correo"], "pw": _HASH})
